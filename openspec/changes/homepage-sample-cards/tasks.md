...
- [ ] 5.1 `npm run dev` 起本地，桌面端检查：未上传时画廊出现、点击展开/折叠/切换正常、拖入文件后画廊消失（**待用户本地验证**）
- [ ] 5.2 浏览器窄化到 ≤640px 检查移动端单列布局（**待用户本地验证**）
- [ ] 5.3 清空 localStorage 后刷新仍正常；执行一次完整上传 → 完成 → 刷新页面，验证恢复态下画廊不出现（**待用户本地验证**）
- [ ] 5.4 DevTools Network 面板确认整个示例画廊的交互全程没有任何 fetch 请求（**待用户本地验证**）
